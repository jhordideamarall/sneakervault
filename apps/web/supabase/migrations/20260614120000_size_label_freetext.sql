-- Fase A #6 — Size free-text (Adidas pecahan: 42 2/3, 38 2/3, dst).
--
-- products.size (numeric) tidak bisa menyimpan "42 2/3". Solusi additive & low-risk:
--   - tambah size_label (text) = nilai yang diketik/ditampilkan (format marketplace)
--   - size (numeric) tetap ada sebagai kunci urut/banding, diisi otomatis via trigger
--   - trigger dua-arah: jalur lama yang kirim `size` numerik tetap jalan (label diisi)
-- SKU tetap kunci match; size hanya display/grouping.

-- 1) Parser label -> numeric (untuk sorting/banding). IMMUTABLE agar bisa dipakai di mana saja.
...
BEGIN
  IF s = '' THEN
    RETURN 0;
  END IF;
  s := replace(s, ',', '.');                       -- "37,5" -> "37.5"

  -- Mixed fraction: "42 2/3"
  ...
  -- Plain int/decimal, mungkin ada teks ekor -> ambil angka di depan.
  -- NB: pakai pola tanpa grup-tangkap; substring(from pattern) mengembalikan
  -- isi grup-tangkap bila ada, jadi '40' bisa salah jadi NULL.
  v := nullif(substring(s from '^[0-9]+\.?[0-9]*'), '')::numeric;
  RETURN coalesce(v, 0);
END;
$$;

-- 2) Kolom size_label (additive). DB kosong saat ini -> backfill aman.
...
-- 3) Trigger dua-arah: sinkron size <-> size_label sebelum tulis.
...
-- size_label wajib ada (trigger menjamin terisi bila size diberikan).
ALTER TABLE public.products ALTER COLUMN size_label SET NOT NULL;

-- 4) get_inventory_page: tambah kolom return size_label. Return type berubah -> drop dulu.
