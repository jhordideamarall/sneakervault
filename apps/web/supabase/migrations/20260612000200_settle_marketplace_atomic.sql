-- Atomic marketplace settlement reconciliation (2-phase: pending -> released).
--
-- Phase 1 (dana belum cair): clears the order AR into Saldo Marketplace (1.1.03)
-- and books the actual marketplace fee delta (invoice.total - net) to 6.1.
-- Phase 2 (dana cair): moves Saldo Marketplace -> chosen bank, records the bank
-- receipt, and marks the invoice paid. Idempotent: re-uploads skip rows already
-- at/after the target phase. Never touches stock (avoids double-count vs packing).
...
BEGIN
  ...
  FOR v_item IN SELECT jsonb_array_elements(p_payload->'items') LOOP
    ...
    IF v_phase = 'pending' THEN
      ...
      v_delta := v_inv.total - v_net;  -- actual extra marketplace cost
    ...
    ELSE  -- released
      ...
      IF v_inv.settlement_status <> 'pending' THEN
        -- Must upload the "belum cair" (pending) settlement first.
        v_skipped := v_skipped + 1;
        CONTINUE;
      END IF;
