-- Atomic POS checkout.
-- Moves the multi-write POS flow (invoice, lines, stock, journals, payment,
-- allocation, bank) into ONE PL/pgSQL transaction so a mid-flow failure rolls
-- back everything instead of leaving partial accounting data.
--
-- Additive only: existing pos.ts still works on its old path until it is
-- rewired to call pos_checkout(). app_post_journal is internal-only (callable
-- by other SECURITY DEFINER functions, never directly by clients).

-- ── Reusable journal poster (mirrors createJournalEntry in journal-engine.ts) ──
...
DECLARE
  ...
  v_pay_lines      jsonb;
BEGIN
  -- 1. Role gate (defense-in-depth; server action already checks).
  ...
  -- 3. Aggregate payload lines per product (qty summed, last price wins) + join products.
  ...
  -- 6. Lines + guarded stock decrement + movements.
  ...
  -- 7. Sales journal (mirrors journalForSalesInvoice, channel=offline).
  ...
  -- 11. Payment journal (mirrors journalForCustomerPayment).
