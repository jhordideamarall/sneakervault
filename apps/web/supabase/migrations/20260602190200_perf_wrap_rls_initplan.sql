...
-- (these are scalar/stable) — only the evaluation count changes, so it gets
-- much faster as data/users grow. Resolves the 8 "Auth RLS Init Plan" advisor
-- warnings plus 77 has_any_role/has_role policies that had the same pattern.
--
-- Fail-closed: runs in one transaction; any malformed rewrite aborts everything.
-- Verified after apply: 0 bare auth.uid / has_any_role remain, 108 policies intact.
DO $$
DECLARE
  r record;
  nq text;
