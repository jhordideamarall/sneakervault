...
--
-- Why: these rows are reference data the journal engine depends on — every
-- journal posting resolves accounts by `code` (e.g. 1.1.04 Piutang, 5.1 HPP).
-- They previously existed ONLY in the remote DB (not in any migration), so a
-- fresh database had no CoA and all journals failed. This makes the seed
-- reproducible for resets / new environments.
--
-- Safe to re-run: ON CONFLICT (code) DO NOTHING and WHERE NOT EXISTS guards mean
-- it never duplicates or overwrites existing rows. Values are derived verbatim
-- from the live DB, not hand-authored.

-- 1) Accounts (code is UNIQUE).
...
-- 2) Parent hierarchy resolved by code (parent_id is a self-FK; never hardcode ids).
--    Idempotent: only touches rows whose parent_id differs from the target.
...
-- 3) Expense categories (CRUD-able; no unique key, so guard by name).
...
FROM (VALUES
  ('Gaji karyawan','6.5','t','10'),
  ('Sewa toko/gudang','6.8','t','20'),
  ('Listrik & internet','6.10','t','30'),
  ('Biaya packing','6.4','t','40'),
  ('Kardus, plastik, bubble wrap','6.4','t','50'),
  ('Biaya admin marketplace','6.1','t','60'),
  ('Biaya iklan Shopee/TikTok/Instagram','6.9','t','70'),
  ('Biaya ongkir/subsidi ongkir','6.3','t','80'),
  ('Biaya transport','6.4','t','90'),
  ('Biaya software/tools','6.11','t','100'),
  ('Biaya refund/komplain','6.11','t','110'),
  ('Biaya service/perbaikan','6.11','t','120'),
  ('Biaya makan/operasional','6.4','t','130'),
  ('Biaya lain-lain','6.11','t','140')
) AS v(name, account_code, is_system, sort_order)
WHERE NOT EXISTS (
  SELECT 1 FROM public.expense_categories e WHERE e.name = v.name
);
