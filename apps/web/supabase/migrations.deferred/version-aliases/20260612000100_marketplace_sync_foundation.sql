...
--   1. CoA 4.1.04 Penjualan Tokopedia (revenue account for Tokopedia channel).
--   2. marketplace_sku_map — bridge marketplace seller SKU <-> system product,
...
ON public.marketplace_sku_map FOR UPDATE
TO authenticated
...
ON public.marketplace_sku_map FOR DELETE
TO authenticated
...
-- ── 3. Extend marketplace_imports (existing settlement-shaped audit table) ─────
...
-- Payload:
-- {
--   invoice_date, channel, customer_name, marketplace_order_id, notes,
--   discount, shipping_fee, admin_fee,
--   lines: [{product_id, qty, unit_price}, ...]
-- }
-- Returns: { skipped, invoice_id, invoice_number, total }
...
BEGIN
  ...
  -- 2. Duplicate-order guard (idempotency by marketplace_order_id).
  ...
  -- 6. Invoice header (status issued, AR open).
  ...
  -- 7. Lines + guarded stock decrement + movements.
  ...
  -- 8. Sales journal (mirrors journalForSalesInvoice for marketplace channel).
