-- Normalize compact half-size labels that came from spreadsheet formatting,
-- e.g. "395" should mean 39.5 for sneaker sizing, not size 395.
CREATE OR REPLACE FUNCTION public.parse_size_to_numeric(p_label text)
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
SET search_path = public, pg_temp
AS $$
DECLARE
  s    text := btrim(coalesce(p_label, ''));
  frac text;
  v    numeric;
BEGIN
  IF s = '' THEN
    RETURN 0;
  END IF;

  s := replace(s, ',', '.');
  s := btrim(regexp_replace(s, '^[^0-9]+', ''));

  IF s = '' THEN
    RETURN 0;
  END IF;

  IF s ~ '^[0-9]{2}5$' THEN
    RETURN substring(s from 1 for 2)::numeric + 0.5;
  END IF;

  IF s ~ '^[0-9]+ +[0-9]+/[0-9]+$' THEN
    frac := split_part(s, ' ', 2);
    RETURN split_part(s, ' ', 1)::numeric
         + (split_part(frac, '/', 1)::numeric / nullif(split_part(frac, '/', 2)::numeric, 0));
  END IF;

  IF s ~ '^[0-9]+/[0-9]+$' THEN
    RETURN split_part(s, '/', 1)::numeric / nullif(split_part(s, '/', 2)::numeric, 0);
  END IF;

  v := nullif(substring(s from '^[0-9]+\.?[0-9]*'), '')::numeric;
  RETURN coalesce(v, 0);
END;
$$;

UPDATE public.products p
SET size = substring(p.size_label from 1 for 2)::numeric + 0.5,
    size_label = substring(p.size_label from 1 for 2) || '.5',
    updated_at = now()
WHERE p.size >= 100
  AND p.size_label ~ '^[0-9]{2}5$'
  AND NOT EXISTS (
    SELECT 1
    FROM public.products existing
    WHERE existing.id <> p.id
      AND existing.sku = p.sku
      AND round(existing.size, 2) = round(substring(p.size_label from 1 for 2)::numeric + 0.5, 2)
  );
