...
import { parseSettlementFile, type SettlementRow } from "@/lib/marketplace/settlement-parsers";
...
export function SettlementImportClient() {
  ...
  const [phase, setPhase] = useState<SettlementPhase>("pending");
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        const parsed = parseSettlementFile(channel, raw);
        if (parsed.length === 0) {
          toast.push("Tidak ada baris settlement terbaca", "error");
          return;
        }
        setRows(parsed);
        startTransition(async () => {
          const r = await reconcileSettlement(channel, phase, parsed);
          setDiff(r);
          setState("review");
        });
      } catch {
  ...
  function handleCommit() {
    startTransition(async () => {
      const r = await commitSettlement({
        channel,
        phase,
        bankAccountId: phase === "released" ? bankId : null,
        settledDate,
    ...
    <div className="space-y-6">
      {state === "upload" && (
        <Card className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h2 className="mb-2 text-lg font-semibold text-white">Rekonsiliasi Settlement</h2>
          <p className="mb-6 max-w-md text-sm text-white/50">
            Upload laporan settlement marketplace. Tahap 1 saat dana belum cair
            (tertahan), tahap 2 saat dana cair ke bank.
          </p>

          {/* Phase selector */}
          <div className="mb-3 flex gap-2 rounded-lg border border-white/[0.06] bg-[#262626] p-1.5">
            <PhaseBtn active={phase === "pending"} onClick={() => setPhase("pending")} label="Belum Cair" sub="Tahap 1 · tertahan" />
            <PhaseBtn active={phase === "released"} onClick={() => setPhase("released")} label="Sudah Cair" sub="Tahap 2 · masuk bank" />
          </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Badge className={phase === "pending" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"}>
                {phase === "pending" ? "Belum Cair" : "Sudah Cair"}
              </Badge>
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleCommit} disabled={pending || diff.summary.apply === 0}>
...
function PhaseBtn({ active, onClick, label, sub }: { active: boolean; onClick: () => void; label: string; sub: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={"flex flex-col items-start rounded-md px-4 py-2 transition-colors " + (active ? "bg-white/[0.1] text-white" : "text-white/45 hover:bg-white/[0.04] hover:text-white/70")}
    >
