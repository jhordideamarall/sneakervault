    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-[#262626] p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-[11px] text-white/40 mb-1.5">Nama lengkap</label>
            <input
              type="text"
              required
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-2.5 pr-10 text-sm text-white/90 placeholder:text-white/20 focus:border-white/[0.15] focus:outline-none transition-colors"
              />
              <button
                type="button"
            ...
            <p className="text-[10px] text-white/20 mt-1">Minimal 6 karakter</p>
          </div>
          <div>
            <label className="block text-[11px] text-white/40 mb-1.5">Role</label>
            <select
              value={form.role}
              onChange={(e) => setForm(f => ({ ...f, role: e.target.value as Role }))}
              className="w-full rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-2.5 text-sm text-white/90 focus:border-white/[0.15] focus:outline-none appearance-none transition-colors capitalize"
            >
              {ROLES.filter(r => r !== "owner").map((role) => (
                <option key={role} value={role} className="bg-[#262626] text-white">{role.replace("_", " ")}</option>
              ))}
            </select>
          </div>
